    ...
    updateConnectionStatus(connected) {
        ...
        if (connected) {
            statusElement.innerHTML = `
                ...
                <span style="color: #27ae60;">Connected - Shared Data</span>
            `;
        } else {
            statusElement.innerHTML = `
                <div style="width: 8px; height: 8px; background: #e74c3c; border-radius: 50%;"></div>
                <span style="color: #e74c3c;">Local Mode Only</span>
            `;
        }
    }
    ...
    handleWebSocketMessage(data) {
        ...
        switch (data.type) {
            case 'initial':
                ...
                this.renderSurveys();
                this.updateStats();
                console.log('Loaded initial data from server:', this.surveys.length, 'surveys');
                break;
    ...
    handleLocalAction(type, data) {
        switch (type) {
            case 'add_survey':
                ...
                this.renderSurveys();
                this.updateStats();
                this.showNotification('Survey added successfully (local only)');
                break;
            ...
            case 'update_survey':
                ...
                if (surveyIndex !== -1) {
                    ...
                    this.renderSurveys();
                    this.updateStats();
                    this.showNotification('Survey updated successfully (local only)');
                }
                break;
            ...
            case 'delete_survey':
                ...
                this.renderSurveys();
                this.updateStats();
                this.showNotification('Survey deleted successfully (local only)');
                break;
            ...
            case 'add_file':
                ...
                this.showNotification('File uploaded successfully (local only)');
                break;
            ...
            case 'delete_file':
                if (this.files[data.surveyId]) {
                    ...
                    this.showNotification('File deleted successfully (local only)');
                }
                break;
        }
    }
